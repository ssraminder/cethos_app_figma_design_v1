# SOP-031 — Adaptation (Standalone LV Step)
...
| **Related** | SOP-029 (master LV framework) · workflow template `lv_adaptation` |
...
This SOP is the step recipe only. Intake, vendor assignment, the QA gate mechanics, delivery and records are defined once in **SOP-029** — this document points to them, not repeats them.
...
- **Out of scope:** a fresh forward translation from the original source (SOP-030); reconciliation (SOP-032); harmonization across languages (SOP-035). Each is its own PO/workflow.
...
2. Assign a **qualified** linguist native/in-region for the **target variant** and the subject field, from the QMS roster per **SOP-029 §6.2** (eligibility gated — never assign an unqualified linguist).
...
- Adapter and reviser competence (ISO 17100 §3.1.3–§3.1.4) held and current in the QMS before assignment (SOP-029 §5).
...
- Sibling translation-type step: **SOP-030** Forward Translation (§5.3.3). Related: **SOP-035** Harmonization (cross-language consistency).
