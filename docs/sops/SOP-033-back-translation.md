# SOP-033 — Back-translation (Standalone LV Step)
...
| **Standard / ISO reference** | ISO 17100:2015 §5.3.2, §5.3.3, §5.3.6; ISPOR COA good practices |
| **Related** | SOP-029 (master LV framework) · workflow template `lv_back_translation` |
...
Because this step produces a translation (target → English), its QA node is a **§5.3.3 bilingual revision by a second person** (per the task instruction for this step). The back-translation is a *diagnostic* artefact for the client's downstream BT review (SOP-034) — Cethos does not judge the forward translation here. Cethos is ISO 17100-**aligned, not certified** (Stage 2 target ~Dec 2026) — never write "certified."
...
- **Out of scope:** forward translation (SOP-030); the source-vs-BT discrepancy review (SOP-034, usually the client's or a separate PO); reconciliation (SOP-032). Each is its own PO/workflow.
...
2. Assign a **qualified** linguist, native in English and fluent in the target language, for the subject field per **SOP-029 §6.2** — and **not** the person who produced the forward translation (independence). Never assign an unqualified linguist.
...
- Back-translator and reviser competence (ISO 17100 §3.1.3–§3.1.4) held and current in the QMS before assignment (SOP-029 §5).
...
- Downstream: **SOP-034** BT Review (compares this back-translation to the source). Related: **SOP-032** Reconciliation, **SOP-030** Forward Translation.
