# SOP-030 — Forward Translation (Standalone LV Step)
...
| **Standard / ISO reference** | ISO 17100:2015 §5.3.1, §5.3.2, §5.3.3, §5.3.6; ISPOR COA good practices |
| **Related** | SOP-029 (master LV framework) · workflow template `translation_only` |
...
This SOP is the step recipe only. Intake, vendor assignment, the QA gate mechanics, delivery and records are defined once in **SOP-029** — this document does not repeat them; it points to them.
...
- **Out of scope:** adaptation to a regional variant (SOP-031), reconciliation (SOP-032), back-translation (SOP-033). Each is its own PO/workflow — do not bolt them on.

## 3. Procedure

**PM**
1. Confirm intake per **SOP-029 §6.1** — create/confirm the order on the `translation_only` template; record instrument, source→target pair, amount, PO#, PM. Check the source, target locale, glossary/reference set and deadline are all present; if anything is missing or ambiguous, request it before assigning.
2. Assign a **qualified** linguist for the source→target pair and subject field from the QMS roster per **SOP-029 §6.2** (eligibility gated on role/language/subject qualification — never assign an unqualified linguist).
...
**Independent reviser (QA step 2) — see SOP-029 §6.4**
...
**PM (release step 3)**
9. On QA approval, assemble the final deliverable and release to the client per **SOP-029 §6.5** (§5.3.6 verification & release). Invoice/close per the client terms (SOP-029 §6.6).
...
- Translator and reviser competence (ISO 17100 §3.1.3–§3.1.4) is held and current in the QMS before assignment (SOP-029 §5).
...
- **Delivery:** released to the client only after the §5.3.3 revision passes; delivery recorded per SOP-029 §6.5 (the §5.3.6 release record). De-identify any PII before anything leaves Cethos.

## 6. Records & retention

Per **SOP-029 §8** (ISO 17100 §6.2 — retain **≥ 5 years**): order/PO record (instrument, pair, PO#, PM); linguist + reviser assignment and eligibility log; source and target files; translator notes/queries; the §5.3.3 revision outcome (reviser identity, timestamp, result); delivery/release record; any client feedback linked to its CAPA record. Source and target files held in controlled storage (vendor-deliveries bucket + project Dropbox per the folder-naming SOP).
...
- **SOP-029** — master LV framework: intake, qualified-vendor assignment, the QA gate, delivery/release, records & retention, and the §5.3.3-vs-§5.3.6 conformance basis (the shared procedures this SOP relies on).
- **LV-standalone-workflow-set-design.md** — §2 ISO conformance basis, §3 task→clause map (row 1), §4 the 3-node structure.
- Sibling translation-type step: **SOP-031** Adaptation (§5.3.3). Downstream LV steps: **SOP-032** Reconciliation, **SOP-033** Back-translation.
