...
async function processQuotePayment(
  ...
  if (childCount && childCount > 0) {
    ...
    // (full money + payment row) + N child work-unit orders ($0). Customer
    // still sees ONE order; emails below fire once on the parent.
    ...
    if (rpcErr || !rpcResult?.parent_order_id) {
      throw new Error(`Fan-out conversion failed: ${rpcErr?.message}`);
    }
    order = {
      id: rpcResult.parent_order_id,
      order_number: rpcResult.parent_order_number,
    };
    const nChildren = Array.isArray(rpcResult.child_orders)
      ? rpcResult.child_orders.length
      : 0;
    console.log(
      `Fan-out: quote ${quote.quote_number} → parent order ${order.order_number} + ${nChildren} work units`,
    );
  } else {
