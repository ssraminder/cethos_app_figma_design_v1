...
//     success: true,
//     assessment_id: uuid,
//     overall_verdict: "pass" | "fail" | "partial" | "insufficient_evidence",
//     result: { overall_reasoning, criteria: { ... } },
//     input_snapshot: { ... }   // for transparency
//   }
//
// Env vars:
//   ANTHROPIC_API_KEY   required
//   ISO17100_MODEL      optional, default 'claude-sonnet-4-6'
// =============================================================================
...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Pull every piece of evidence we have for this vendor.
    ...
    const { data: apps } = await sb
      ...
      .limit(1);
    const application: ApplicationRow | null = (apps?.[0] as ApplicationRow | undefined) ?? null;
    ...
    if (application) {
      const { data: r } = await sb
        ...
        .order("created_at", { ascending: false });
      referenceRows = (r ?? []) as ReferenceRow[];
    }
    ...
    // Build the snapshot we pass to the LLM. This is also what gets
    // stored in the assessment row — frozen evidence so an older
    // assessment is reproducible even if the underlying rows change.
    const snapshot = {
      vendor: {
      ...
      language_pairs: (pairs ?? []).map((p: LanguagePairRow) => ({
...
${JSON.stringify(snapshot, null, 2)}`;
    ...
    // Pull the JSON out — Claude usually returns clean JSON when asked,
    // but defensively strip ``` fences if present.
    ...
    const overall = String((parsed as { overall?: string }).overall ?? "");
