// ============================================================================
// send-invoice-email v3
// Apr 16, 2026
// CHANGES FROM v2:
//   - Stop creating a Stripe Checkout Session at email-send time. Those
//     sessions expire after Stripe's 24h cap — any customer who clicked
//     after a day got the "session timed out" page.
//   - Email "Pay Online Now" button now points at the durable
//     start-invoice-payment edge function, which creates a fresh Stripe
//     session on every click. The link in the email never expires.
//   - Still creates the payment_requests row so admins can track the
//     outstanding request. stripe_payment_link_* stay null until the
//     customer clicks (then start-invoice-payment fills them in).
// ============================================================================
...
function fmt(n: any, currency = 'CAD'): string {
  if (n == null) return `$0.00 ${currency}`;
  return `$${parseFloat(String(n)).toFixed(2)} ${currency}`;
}
...
serve(async (req) => {
  ...
  try {
    ...
    // ── 5. Create payment_request row — no Stripe session yet ──────────────
    // Stripe session will be created on-click by start-invoice-payment so
    // the link in the email doesn't expire after 24h.
    let paymentRequestId: string | null = null;
    let payUrl: string | null = null;
    const balanceDue = parseFloat(invoice.balance_due || 0);

    if (balanceDue > 0) {
      const { data: pr, error: prErr } = await sb.from('payment_requests').insert({
      ...
      if (prErr) {
        console.warn('payment_requests insert failed:', prErr.message);
      } else {
        paymentRequestId = pr?.id || null;
      }

      if (paymentRequestId) {
        payUrl = `${supabaseUrl}/functions/v1/start-invoice-payment?req=${paymentRequestId}`;
      }
    }
    ...
    const stripeButtonHtml = payUrl ? `
      <div style="margin:24px 0;text-align:center;">
        <a href="${payUrl}" style="display:inline-block;padding:14px 32px;background:#2563eb;color:#fff;text-decoration:none;border-radius:6px;font-size:15px;font-weight:600;">
          \ud83d\udcb3 Pay Online Now \u2014 ${fmt(balanceDue, cur)}
        </a>
    ...
    const linesSummaryHtml = (lines || []).map((l: any) => `
      <tr>
        ...
        <td style="padding:8px 12px;border-bottom:1px solid #f1f5f9;font-size:13px;text-align:right;">${fmt(l.line_total, cur)}</td>
      </tr>`).join('');
...
<div style="max-width:600px;margin:0 auto;background:#fff;">
  ...
  <div style="background:#2563eb;padding:12px 32px;">
    ...
    <p style="margin:4px 0 0 0;color:#bfdbfe;font-size:13px;">From ${branch.legal_name}${cur !== 'CAD' ? ` \u00b7 Currency: ${cur}` : ''}</p>
  </div>
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:24px;border:1px solid #e2e8f0;border-radius:6px;overflow:hidden;">
      ...
      <tfoot>
        <tr style="background:#f8fafc;">
          <td style="padding:8px 12px;font-size:13px;color:#64748b;">${branch.tax_label || 'GST'} (${(parseFloat(invoice.tax_rate || 0.05) * 100).toFixed(0)}%)</td>
          <td style="padding:8px 12px;font-size:13px;text-align:right;color:#64748b;">${fmt(invoice.tax_amount, cur)}</td>
        </tr>
        <tr style="background:#eff6ff;">
          <td style="padding:10px 12px;font-size:15px;font-weight:700;color:#1e293b;">Total</td>
          <td style="padding:10px 12px;font-size:15px;font-weight:700;text-align:right;color:#1e293b;">${fmt(invoice.total_amount, cur)}</td>
        </tr>
        ${parseFloat(invoice.amount_paid || 0) > 0 ? `
        <tr>
          <td style="padding:8px 12px;font-size:13px;color:#16a34a;">Amount Paid</td>
          <td style="padding:8px 12px;font-size:13px;text-align:right;color:#16a34a;">${fmt(invoice.amount_paid, cur)}</td>
        </tr>` : ''}
        <tr style="background:#fef2f2;">
          <td style="padding:10px 12px;font-size:15px;font-weight:700;color:#dc2626;">Balance Due</td>
          <td style="padding:10px 12px;font-size:15px;font-weight:700;text-align:right;color:#dc2626;">${fmt(invoice.balance_due, cur)}</td>
        </tr>
      </tfoot>
    </table>
  ...
  <div style="background:#f1f5f9;padding:16px 32px;border-top:1px solid #e2e8f0;text-align:center;">
    ...
    console.log(`Sending invoice ${invoice.invoice_number} (${cur}) to ${sentToList}`);
    ...
    if (!brevoRes.ok) {
      ...
      await sb.from('invoice_email_log').insert({
        invoice_id, customer_id: customer.id, sent_to: sentToList,
        sent_by_staff_id: staff_id || null, subject: emailPayload.subject,
        custom_message: noteText || null, status: 'failed', error_message: errText,
        stripe_payment_url: payUrl, payment_request_id: paymentRequestId,
      });
    ...
    await sb.from('invoice_email_log').insert({
      invoice_id, customer_id: customer.id, sent_to: sentToList,
      sent_by_staff_id: staff_id || null, subject: emailPayload.subject,
      custom_message: noteText || null, status: 'sent',
      stripe_payment_url: payUrl, payment_request_id: paymentRequestId,
    });
    ...
    await sb.from('customer_invoices').update({
      last_emailed_at: now, last_emailed_to: sentToList,
      email_sent_count: newEmailCount, status: newStatus, updated_at: now,
    }).eq('id', invoice_id);

    if (staff_id) {
      await sb.from('staff_activity_log').insert({
        staff_id, action_type: 'invoice_email_sent', entity_type: 'invoice', entity_id: invoice_id,
        details: { invoice_number: invoice.invoice_number, sent_to: sentToList, currency: cur, pay_link: !!payUrl, payment_request_id: paymentRequestId },
      });
    }

    console.log(`\u2705 Invoice ${invoice.invoice_number} (${cur}) emailed to ${sentToList} (send #${newEmailCount})`);
    return jsonResp({
      success: true, invoice_number: invoice.invoice_number, sent_to: sentToList,
      email_count: newEmailCount, stripe_payment_url: payUrl,
      payment_request_id: paymentRequestId, new_status: newStatus, currency: cur,
    });
