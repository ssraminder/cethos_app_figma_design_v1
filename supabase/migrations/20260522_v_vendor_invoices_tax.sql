-- View unifying XTRF vendor-invoice cache + portal vendor_payables with
-- correctly-computed CAD tax for ITC (Line 106) reporting.
--
-- Why a view: xtrf_vendor_invoice_cache.gross_cad is corrupted (set to 1
-- for many rows). The CSV staging table has authoritative tax amounts
-- per invoice_number. This view joins the two and exposes one
-- consistent row per vendor invoice with branch attribution.
--
-- Branch attribution (XTRF cache.branch text → branches.id):
--   'Cethos Solutions Inc.'                                     → 1
--   '12537494 Canada Inc.'                                      → 2
--   'Amrita Arora'                                              → 4
--   'Cethos Solutions Inc. - Linguistic Validation Division'    → 6
--   other / null                                                → NULL (unassigned)
...
WITH cache_rows AS (
  SELECT
    ...
    -- Prefer CSV tax (parsed) when available, else compute via implied
    -- FX rate from netto_cad / total_netto.
    COALESCE(
