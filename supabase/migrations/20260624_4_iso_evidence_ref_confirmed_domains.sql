-- Phase 2 of the referee-form enhancement: expose reference-confirmed domains on
-- the applicant ISO-evidence view so the reviewer guide + approval can credit a
-- domain that a professional reference confirmed (policy 2026-06-23: references
-- can establish §6.1.6 domain competence for ANY declared domain, incl. COA).
--
-- Append-only: adds one output column `ref_confirmed_domains text[]` (union of
-- reference_confirmed_domains across received references). All prior columns
-- unchanged. Recreated in full (CREATE OR REPLACE) from 20260624_1.
CREATE OR REPLACE VIEW public.cvp_application_iso_evidence AS
 WITH refs AS (
         ...
         SELECT r.application_id, array_agg(DISTINCT dom) AS confirmed_domains
           FROM cvp_application_references r, unnest(r.reference_confirmed_domains) dom
          WHERE r.status = 'received'::text AND r.reference_confirmed_domains IS NOT NULL
 ...
 SELECT a.id AS application_id,
    ...
    vd.degree_doc,
    COALESCE(refdoms.confirmed_domains, '{}'::text[]) AS ref_confirmed_domains
   FROM cvp_applications a
     LEFT JOIN refs r ON r.application_id = a.id
     LEFT JOIN refsx ON refsx.application_id = a.id
     LEFT JOIN refdoms ON refdoms.application_id = a.id
