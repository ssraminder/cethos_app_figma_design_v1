-- Extend the first-party generator to combine portal vendor_payables with
-- pre-2023 legacy_supplier_invoice_summary, giving the true earliest engagement
-- date + full job/volume totals. Applied to prod via MCP 2026-06-18.
-- Supersedes 20260618_qms_build_first_party_experience_promote.sql.
...
BEGIN
  ...
  IF p_dry_run THEN
    RETURN jsonb_build_object('found', true, 'dry_run', true, 'vendor_id', p_vendor_id,
      ...
      'total', v_total, 'currencies', v_currencies, 'legacy_invoices', COALESCE(v_leg_n,0), 'note', v_note);
  END IF;
