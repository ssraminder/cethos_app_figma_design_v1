-- Data-integrity fix: a combo's status='approved' is NOT valid evidence of a
-- passed test. A General test pass CASCADES 'approved' onto every declared domain
-- (cvp-assess-test), and a backfill stamped others — all with no test assigned
-- (test_id null), no submission, no score. Only count a combo as PASSED when it is
-- genuinely GRADED (real test_submission_id + ai_score) AND approved. So
-- real_passed_combos + passed_domains are redefined to require a graded submission.
...
comb as (
  select application_id,
    ...
    count(*) filter (where status = 'approved' and test_submission_id is not null and ai_score is not null) real_passed_combos,
    ...
    string_agg(distinct domain, ',') filter (where status = 'approved' and test_submission_id is not null and ai_score is not null) passed_domains
...
select
  ...
  coalesce(c.passed_domains, '') as passed_domains
