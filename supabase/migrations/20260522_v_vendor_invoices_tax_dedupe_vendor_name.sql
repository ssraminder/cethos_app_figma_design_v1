-- Two fixes to v_vendor_invoices_tax:
--
-- 1. CSV dedup: xtrf_csv_vendor_invoices_2026_05_21 has duplicate invoice
--    numbers ("1" × 45, "2" × 23, "3" × 18, ...) because XTRF reuses short
--    sequential numbers across vendors. The previous LEFT JOIN multiplied
--    cache rows up to ~2x, inflating Q3 2025 from 438 to 957 rows and
--    bumping ITC totals from real $7,020.50 → reported $7,080.90.
--    Use LATERAL with LIMIT 1 so each cache row resolves to at most one
--    CSV tax-amount lookup.
--
-- 2. Vendor name: cache.vendor_name is null on most recent rows but
--    cache.provider_id maps cleanly to vendors.xtrf_vendor_id (98%
--    coverage). Prefer vendors.full_name when available, fall back to
--    cache.vendor_name, then '(unknown)'. Q3 2025 went from "1 vendor
--    (unknown)" to 45 distinct vendors after this change.
...
WITH cache_rows AS (
  SELECT
    ...
    COALESCE(NULLIF(TRIM(v.full_name), ''), NULLIF(TRIM(c.vendor_name), ''), '(unknown)') AS vendor_name,
  ...
  LEFT JOIN public.vendors v ON v.xtrf_vendor_id = c.provider_id
  LEFT JOIN LATERAL (
    SELECT NULLIF(regexp_replace(COALESCE(csv.tax_amount, ''), '[^0-9.\-]', '', 'g'), '')::numeric AS csv_tax
    FROM public.xtrf_csv_vendor_invoices_2026_05_21 csv
    WHERE COALESCE(NULLIF(csv.invoice_no, ''), csv.internal_number) =
          COALESCE(c.final_number, c.draft_number, c.internal_number)
    ORDER BY csv.invoice_date_final DESC NULLS LAST
    LIMIT 1
  ) csv_match ON true
),
payable_rows AS (
