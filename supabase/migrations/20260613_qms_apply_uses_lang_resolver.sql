-- Point the apply function's language-pair resolution at qms_resolve_language_id
-- (exact→alias→base-code fallback) + DISTINCT so region-suffixed vendor codes
-- resolve and don't collide. Only the language-pair block changed vs the
-- 20260612 definition.
...
BEGIN
  ...
  END IF;
  IF r.applied_at IS NOT NULL THEN RAISE EXCEPTION 'result % already applied at %', p_result_id, r.applied_at; END IF;
  ...
  IF NOT EXISTS (
    ...
    IF NOT FOUND THEN RAISE EXCEPTION 'vendor % has no current portal NDA signature to bridge', r.vendor_id; END IF;
  ...
  -- Language pairs via the region-aware resolver; DISTINCT collapses region
  -- variants that map to the same base language (EN-US/EN-GB/EN → en).
  ...
  SELECT DISTINCT v_translator_qual, sl, tl, 'source_to_target'::qms.pair_direction, v_evidence_id,
         'Resolved from vendor_language_pairs', p_acting_user_id
  FROM public.vendor_language_pairs p
  CROSS JOIN LATERAL public.qms_resolve_language_id(p.source_language) AS sl
  CROSS JOIN LATERAL public.qms_resolve_language_id(p.target_language) AS tl
  WHERE p.vendor_id = r.vendor_id AND COALESCE(p.is_active, true)
