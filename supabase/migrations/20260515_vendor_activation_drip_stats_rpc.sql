-- Stats RPC for the admin vendors-list Activation Drip Progress accordion.
-- Single round-trip; no schema changes. "Activated" = passes the same
-- gates the cron uses to skip a vendor: (vendor_type='agency' OR has CV)
-- AND has a current NDA signature.
...
BEGIN
  ...
  SELECT COUNT(*)
   ...
   WHERE nl.event_type = 'vendor_activation_email'
     ...
     AND nl.recipient_id IN (
       SELECT DISTINCT recipient_id FROM public.notification_log
        WHERE event_type = 'vendor_activation_email' AND recipient_id IS NOT NULL
     );
...
GRANT EXECUTE ON FUNCTION public.get_vendor_activation_drip_stats() TO authenticated;
