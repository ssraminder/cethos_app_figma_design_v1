-- First-party experience evidence generator. Cethos's own payment/PO records
-- (vendor_payables) are first-party documented evidence of professional
-- translation experience — stronger than a self-declared CV. This RPC turns a
-- vendor's non-voided payables into:
--   * a qms.professional_experience row (ISO 17100 §3.1.4(b)/(c) / VM-001 §5.5)
--   * a verified qms.competence_evidence row (method='first_party_records')
-- Idempotent: re-running replaces the prior auto-generated rows for the vendor.
-- p_dry_run=true returns the computed summary without writing.
-- Applied to prod via MCP 2026-06-18; reachable via manage-qms-evidence
-- (action='build_first_party') from the admin QMS tab.
...
BEGIN
  ...
  SELECT id INTO v_qual_id FROM qms.role_qualifications
    WHERE vendor_id = p_vendor_id AND role_type_id = v_role_id AND status = 'qualified' LIMIT 1;
  SELECT id INTO v_type_id FROM qms.evidence_types WHERE code = 'documented_translation_experience';

  -- Idempotency: drop prior auto-generated rows for this vendor.
  ...
  RETURN jsonb_build_object('found', true, 'dry_run', false, 'vendor_id', p_vendor_id,
    ...
    'evidence_id', v_evidence_id, 'professional_experience_id', v_pe_id, 'role_qualification_id', v_qual_id);
END $$;
