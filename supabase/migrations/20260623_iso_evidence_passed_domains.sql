-- Testing-first (IQVIA): expose the domains an applicant actually PASSED a test
-- in (status='approved'), so the reviewer guide can credit a passed domain test
-- as domain qualification (§6.1.6) instead of only crediting a certificate. The
-- guide was telling reviewers to "trim medical/life-sci/pharma" for applicants
-- who had PASSED those domain tests. Additive — passed_domains appended after the
-- existing columns (CREATE OR REPLACE column-order rule).
...
comb as (
  select application_id,
    ...
    string_agg(distinct domain, ',') filter (where status = 'approved') passed_domains
