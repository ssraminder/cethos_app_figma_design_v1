-- Expose the matched applicant-vendor id in the recruitment ISO evidence so the
-- review page can deep-link to the proven manual upload+screen path (admin vendor
-- QMS tab). This is the manual route for staff to upload documents an applicant
-- emailed in: open the vendor QMS tab → Add document → AI-screens → surfaces back
-- in the recruitment ISO panel. Appends applicant_vendor_id.
