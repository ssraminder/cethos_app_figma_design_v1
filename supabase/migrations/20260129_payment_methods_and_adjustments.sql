...
CREATE TABLE IF NOT EXISTS payment_methods (
  ...
  icon VARCHAR(50), -- Icon name for UI (e.g., 'cash', 'card', 'link')
...
-- ----------------------------------------------------------------------------
-- 2. Quote Adjustments Table (Discounts & Surcharges)
-- ----------------------------------------------------------------------------
CREATE TABLE IF NOT EXISTS quote_adjustments (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  quote_id UUID NOT NULL REFERENCES quotes(id) ON DELETE CASCADE,
  adjustment_type VARCHAR(50) NOT NULL, -- 'discount' or 'surcharge'
  value_type VARCHAR(20) NOT NULL, -- 'percentage' or 'fixed'
...
-- ----------------------------------------------------------------------------
-- 3. Add Payment Method Column to Quotes
-- ----------------------------------------------------------------------------
...
-- payment_methods - public read for active methods
...
-- quote_adjustments - staff can manage
...
-- ----------------------------------------------------------------------------
-- 5. Helper Function: Calculate Quote Adjustments Total
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
COMMENT ON TABLE payment_methods IS 'Available payment methods for quotes and orders';
COMMENT ON TABLE quote_adjustments IS 'Discounts and surcharges applied to quotes by staff';
COMMENT ON COLUMN quotes.payment_method_id IS 'Selected payment method for this quote';
COMMENT ON COLUMN quotes.payment_confirmed_at IS 'Timestamp when offline payment was confirmed by staff';
COMMENT ON COLUMN quotes.payment_confirmed_by_staff_id IS 'Staff member who confirmed the payment';
