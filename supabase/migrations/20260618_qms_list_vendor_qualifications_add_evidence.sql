-- Surface the EVIDENCE (proof) behind each qualification on the admin QMS tab.
-- An auditor needs to see not just "qualified" but the documented proof: the
-- degree/CV record, who/how it was verified, the verbatim basis, file + hash.
-- Add competence_evidence per role qualification to the RPC. Additive.
