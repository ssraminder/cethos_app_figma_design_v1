-- Append reference-derived experience signals so the reviewer guide can reason
-- about ISO 17100 §3.1.4 route (c): earliest reference-confirmed start year →
-- documented years of experience, and count of positive (would-work-again) refs.
-- (Full view body is re-stated; only the refsx CTE + 3 trailing columns are new.)
