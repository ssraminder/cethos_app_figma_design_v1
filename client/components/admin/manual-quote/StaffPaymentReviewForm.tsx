  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-6">
        <h3 className="text-sm font-semibold text-gray-900 mb-4">Quote Summary</h3>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {reviewData.customer?.companyName && (
              <p className="text-xs text-gray-600">{reviewData.customer.companyName}</p>
            )}
            <p className="text-xs text-gray-600">{reviewData.customer?.email}</p>
            <p className="text-xs text-gray-600">{reviewData.customer?.phone}</p>
            <p className="text-xs text-gray-500 mt-1">
              Entry Point: {reviewData.entryPoint?.replace("staff_", "").replace("_", " ").toUpperCase()}
            </p>
          </div>
          ...
          {reviewData.quote?.targetLanguageId && (
            <div>
              ...
              <p className="text-sm text-gray-900">
                {reviewData.quote.sourceLanguageId ? "Language pair selected" : "Target language selected"}
              </p>
            </div>
          )}
          ...
          {reviewData.files && reviewData.files.length > 0 && (
            <div>
              ...
              <p className="text-sm text-gray-900">
                {reviewData.files.length} file{reviewData.files.length > 1 ? "s" : ""} attached
              </p>
            </div>
          )}
          ...
          <div>
            ...
            <div className="bg-gray-50 rounded-md p-3">
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="font-medium">${reviewData.pricing?.subtotal?.toFixed(2) || "0.00"}</span>
              </div>
              {reviewData.pricing?.rushFee > 0 && (
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-gray-600">Rush Fee</span>
                  <span className="font-medium">${reviewData.pricing.rushFee.toFixed(2)}</span>
                </div>
              )}
              {reviewData.pricing?.deliveryFee > 0 && (
                <div className="flex justify-between text-sm mb-1">
                  <span className="text-gray-600">Delivery</span>
                  <span className="font-medium">${reviewData.pricing.deliveryFee.toFixed(2)}</span>
                </div>
              )}
              <div className="flex justify-between text-sm mb-1">
                <span className="text-gray-600">Tax</span>
                <span className="font-medium">${reviewData.pricing?.taxAmount?.toFixed(2) || "0.00"}</span>
              </div>
              <div className="flex justify-between text-base font-bold pt-2 border-t border-gray-300 mt-2">
                <span>Total</span>
                <span className="text-indigo-600">${reviewData.pricing?.total?.toFixed(2) || "0.00"}</span>
              </div>
            </div>
          </div>
          ...
          <label className="flex items-start gap-3 cursor-pointer">
            ...
            <div className="flex-1">
              ...
              <p className="text-xs text-blue-700 mt-1">
                Customer will receive an email with a secure payment link to complete the order online.
      ...
      {selectedMethod && !selectedMethod.is_online && selectedMethod.requires_staff_confirmation && (
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-amber-700 mt-1">
                You'll need to manually confirm this payment after the customer pays.
                Quote will remain in "Awaiting Payment" status until confirmed.
              </p>
            </div>
          </div>
        </div>
