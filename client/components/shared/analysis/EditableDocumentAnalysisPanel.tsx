  ...
  const loadDropdownOptions = async () => {
    try {
      const [languagesRes, docTypesRes] = await Promise.all([
        supabase.from("languages").select("id, name, code").eq("is_active", true).order("name"),
    ...
    try {
      const { error: correctionError } = await supabase
        ...
        .insert({
          ...
          knowledge_base_comment: submitToKnowledgeBase ? knowledgeBaseComment : null,
                ...
                <div className="px-4 py-3 border-t space-y-3 text-sm bg-gray-50">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="col-span-2">
                      <p className="text-xs text-gray-600 mb-1">Billable Pages</p>
                      {currentEditState.isEditing ? (
                        <input
                          type="number"
                          step="0.5"
                ...
                <div className="px-4 py-3 border-t space-y-2 text-sm bg-gray-50">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium text-gray-900">
                      ${currentAnalysis.base_rate?.toFixed(2) || "0.00"}/page
                    </span>
                  </div>
                  ...
                  <div className="border-t border-gray-200 pt-2 mt-2 flex justify-between font-semibold text-blue-700">
                    ...
                    <span>${currentAnalysis.line_total?.toFixed(2) || "0.00"}</span>
                  </div>
                </div>
              )}
            </div>
